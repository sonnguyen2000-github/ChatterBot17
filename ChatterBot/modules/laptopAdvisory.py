...
def get_laptop_response(my_bot, request):
    ...
    if 'asus' in userText:
       ...
       with open('G:\Desktop\Document20201\AI\ChatterBot17\ChatterBot\ReadUser.json','w', encoding='utf-8') as user_dumped :
            json.dump(user,user_dumped,ensure_ascii = False) 
 
    number = [int(s) for s in userText.split() if s.isdigit()]   
    n = (len(number))
    with open('G:\Desktop\Document20201\AI\ChatterBot17\ChatterBot\ReadUser.json', 'r', encoding='utf-8') as json_file :
